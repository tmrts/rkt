...
var (
	cmdAppEject = &cobra.Command{
		Use: "eject [--app=APPNAME] UUID",
		Run: ensureSuperuser(runWrapper(runEject)),
	}
)

func init() {
	cmdApp.AddCommand(cmdAppEject)
	cmdAppEject.Flags().StringVar(&flagAppName, "app", "", "name of the app")
}

func runEject(cmd *cobra.Command, args []string) (exit int) {
	if len(args) < 1 {
		cmd.Usage()
		return 1
	}
	...
	if err = stage0.Nsenter(p.path(), podPID, stage1RootFS, []string{"/bin/systemctl", "mask", (*appName).String()}); err != nil {
	...
	if err := os.RemoveAll(filepath.Join(stage1RootFS, "opt/stage2/", (*appName).String())); err != nil {
		stderr.PrintE("ejecting app failed", err)
		return 1
	}

	if err = stage0.Nsenter(p.path(), podPID, stage1RootFS, []string{"/bin/systemctl", "daemon-reload"}); err != nil {
		stderr.PrintE("nsenter entrypoint failed", err)
		return 1
	}
