...
var (
	cmdAppStart = &cobra.Command{
		Use: "start [--app=APPNAME] UUID",
		Run: ensureSuperuser(runWrapper(runStart)),
	}
)

func init() {
	cmdApp.AddCommand(cmdAppStart)
	cmdAppStart.Flags().StringVar(&flagAppName, "app", "", "name of the app")
}

func runStart(cmd *cobra.Command, args []string) (exit int) {
	if len(args) < 1 {
		cmd.Usage()
		return 1
	}
	...
	if err = stage0.Nsenter(p.path(), podPID, stage1RootFS, []string{"/bin/systemctl", "start", "reaper-" + (*appName).String()}); err != nil {
		stderr.PrintE("nsenter entrypoint failed", err)
		return 1
	}
	if err = stage0.Nsenter(p.path(), podPID, stage1RootFS, []string{"/bin/systemctl", "start", (*appName).String()}); err != nil {
		stderr.PrintE("nsenter entrypoint failed", err)
		return 1
	}
