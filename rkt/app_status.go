...
var (
	cmdAppStatus = &cobra.Command{
		Use: "status [--app=APPNAME] UUID",
		Run: ensureSuperuser(runWrapper(runAppStatus)),
	}
)

func init() {
	cmdApp.AddCommand(cmdAppStatus)
	cmdAppStatus.Flags().StringVar(&flagAppName, "app", "", "name of the app")
}
...
func runAppStatus(cmd *cobra.Command, args []string) (exit int) {
	...
	if err = stage0.Nsenter(p.path(), podPID, stage1RootFS, []string{"/bin/systemctl", "status", (*appName).String()}); err != nil {
